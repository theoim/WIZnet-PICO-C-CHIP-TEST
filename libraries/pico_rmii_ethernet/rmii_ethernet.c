...
#define PICO_MII_ETHERNET_TX_CLK_PIN (rmii_eth_netif_config.tx_clk_pin)
...
static err_t netif_rmii_ethernet_output(struct netif *netif, struct pbuf *p)
{
    ...
    for (int i = 0; i < 7 * 2; i++) {
        tx_frame_bits[index++] = 0x10 | 0x5;
    }

    tx_frame_bits[index++] = 0x10 | 0x5;
    tx_frame_bits[index++] = 0x10 | 0xD;
...
static void netif_rmii_ethernet_rx_dv_falling_callback(uint gpio, uint32_t events) {
    if ((PICO_RMII_ETHERNET_RX_PIN + 4) == gpio) {
        pio_sm_set_enabled(PICO_RMII_ETHERNET_PIO, PICO_RMII_ETHERNET_SM_RX, false);
        dma_channel_abort(rx_dma_chan);
        gpio_set_irq_enabled_with_callback(PICO_RMII_ETHERNET_RX_PIN + 4, GPIO_IRQ_EDGE_FALL, false, netif_rmii_ethernet_rx_dv_falling_callback);
    }
}
...
static err_t netif_rmii_ethernet_low_init(struct netif *netif) {
    ...
    mii_ethernet_phy_tx_init(PICO_RMII_ETHERNET_PIO, PICO_RMII_ETHERNET_SM_TX, tx_sm_offset, PICO_RMII_ETHERNET_TX_PIN, PICO_MII_ETHERNET_TX_CLK_PIN);
    ...
    netif_rmii_ethernet_mdio_write(phy_address, 4, 0x0061);
    netif_rmii_ethernet_mdio_write(phy_address, 0, 0x1100);
...
void netif_rmii_ethernet_poll() {
    ...
    if (dma_channel_is_busy(rx_dma_chan)) {
        ...
        int      frame_search_len = (int)sizeof(rx_frame);

        for (int i = 0; i < 16 && i < frame_search_len; i++) {
            if (rx_frame[i] == 0xD5) {
                frame_data       = rx_frame + i + 1;
                frame_search_len = (int)sizeof(rx_frame) - i - 1;
                break;
            }
        }

        uint rx_frame_length = ethernet_frame_length(frame_data, frame_search_len);

        if (rx_frame_length) {
            struct pbuf *p = pbuf_alloc(PBUF_RAW, rx_frame_length, PBUF_POOL);

            if (p != NULL) {
                pbuf_take(p, frame_data, rx_frame_length);
        ...
        mii_ethernet_phy_rx_init(PICO_RMII_ETHERNET_PIO, PICO_RMII_ETHERNET_SM_RX, rx_sm_offset, PICO_RMII_ETHERNET_RX_PIN);

        gpio_set_irq_enabled_with_callback(PICO_RMII_ETHERNET_RX_PIN + 4, GPIO_IRQ_EDGE_FALL, true, &netif_rmii_ethernet_rx_dv_falling_callback);
    }
